" ~tan/.vim/vimrc
" ~tan/.vim/vundle.vimrc
" ~tan/.vim/mac-alt-map.vimrc

" runtimepath管理 "{{{
set runtimepath-=~tan/.vim
set runtimepath+=~tan/.vim
set runtimepath-=~tan/.vim/bundle/vundle.vim
set runtimepath+=~tan/.vim/bundle/vundle.vim
"}}}
...
function! W_set_colorscheme(color)
    ...
    if s:W_TRUE == W_source("~tan/.vim/bundle/rainbow/plugin/rainbow.vim")
        RainbowLoad
    endif
    " }}}3
endfunction
...
" Vundle是一个管理插件的插件
" 这里配置文件里面包含了各个插件的配置
source ~tan/.vim/bundle/vundle.vim/vundle.vimrc
" }}}1
...
if has("gui_running") && has("macunix")
    ...
    source ~tan/.vim/mac-alt-map.vimrc
endif
" }}}2

" Quick Edit/Source vimrc{{{
command! EditRc   :tabnew ~tan/.vim/vimrc
command! SourceRc :source ~tan/.vim/vimrc
"}}}
